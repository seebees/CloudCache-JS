...
var CloudCache = function(a_key, s_key, service_url) {
	...
	this.createRequest = function(CloudCacheCmd, path, callback, additionalHeaders) {
		debug('createRequest:' + CloudCacheCmd);
	...
	this.createRequest(
			...
			function(response) {				
				switch ( response.statusCode ) {
					...
					case 202:
						//joy, we have data
						self._response(response, callback, event, data);
						break;
					case 404:
						...
						self._response(response, null, event, data);
						break;
					default:
						self._response(response, null, event, data, true);
						break;
				};
			},
			headers
...
CloudCache.prototype._response = function(response, callback, request_event, request_data, isError) {
	var response_data = '',
		self = this;
	...
	response.on('data', function(chunk) {
		...
		if ( typeof callback === 'function' ) {
			callback.call(self,response_data);
		}
...
CloudCache.prototype.exists = function(id, callback, encoding){
	return this.get(id, function(ret){
		...
		this.emit('success',!!ret, ret);
		this.emit(id,'EXISTS', !!ret, ret);
	},encoding);
};
...
CloudCache.prototype.myusage = function(callback){
	return this.transport('myusage', '/myusage', function(data){
		if ( typeof callback === 'function' ) {
			callback(data);
		}
		this.emit('success',data);
...
CloudCache.prototype.flush = function(callback) {
	return this.transport('flush', '/flush', function(data){
		if ( typeof callback === 'function' ) {
			callback(data);
		}
		this.emit('success',data);

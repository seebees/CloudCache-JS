...
var CloudCache = function(a_key, s_key, service_url) {
	...
	this.transport = function(cmd, path, callback, data, encoding, headers) {
		...
		debug('Request:' + cmd + ' for path:' + path + ' with data:\r\n' + data);
		...
		req = http.request({
				...
				if ( response.statusCode in http_success ) {
					...
					self.response(response, '404', function(response_data){
						debug(
							'404: Event:' + event + 
							'\r\nRequest Data:' + data +  
							'\r\nRequestHeaders:' + util.inspect(headers) + 
							'\r\nResponse Data:\r\n' + response_data
						);
					});
				} else {
					...
					//Yes, yes, I am hijacking the third paramater
					self.response(response, 'error', {'event':event, 'data':data});
				}
...
CloudCache.prototype.response = function(response, event, callback) {
	...
	response.on('data', function(chunk) {
		...
		if ( event !== 'error' ) {
			...
			//TODO should I be emmiting these?  if yes, should I also emit on the ID?
			self.emit(event, response_data);
		} else {
			self.emit('error', 
